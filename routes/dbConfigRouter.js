...
router.get("/database-views/:name/:schema", async (req, res) => {
  try {
    const { name, schema } = req.params;
    console.log("Fetching views for database:", name, "schema:", schema);
    ...
    const sql = `SELECT table_name FROM information_schema.tables WHERE table_schema = $1 AND table_type = 'VIEW' ORDER BY table_name;`;
    const result = await db.executeQuery(sql, [schema]);
    ...
    return res.status(200).send({
      status: "Success",
      message: "List of views for database and schema",
      result: result.rows.map(row => row.table_name),
    });
  } catch (error) {
    console.log("error:", error);
